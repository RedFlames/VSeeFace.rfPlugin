...
public static class UnityHelper
{
    ...
    public static void DrawRect2D (RectTransform rt, Color c, Vector3 offset = new(), Camera cam = null)
    {
        cam ??= Camera.main;
        ...
        DrawLine2D(corners[0], corners[1], c, pos, cam);
        DrawLine2D(corners[2], corners[1], c, pos, cam);
        DrawLine2D(corners[2], corners[3], c, pos, cam);
        DrawLine2D(corners[0], corners[3], c, pos, cam);
    }

    public static void DrawLine2D (Vector3 a, Vector3 b, Color col, Vector3 offset = new(), Camera cam = null)
    {
        cam ??= Camera.main;

        var rayA = cam.ScreenPointToRay(a + offset);
        
        Vector3 worldA = rayA.origin + rayA.direction * .01f;
        
        var rayB = cam.ScreenPointToRay(b + offset);
        ...
        //GizmosLibraryPlugin.GizmosAPI.DrawWithReference(cam.transform, () =>
        {
